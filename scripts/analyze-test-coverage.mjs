...
class TestCoverageAnalyzer {
  ...
  async analyzeCoverage(): Promise<{
    ...
    try {
      ...
      execSync('npx vitest --coverage --run --reporter=json --outputFile=coverage-results.json', {
        stdio: 'pipe',
        cwd: this.projectRoot
      })

      // Read coverage results
      if (existsSync(resolve(this.projectRoot, 'coverage-results.json'))) {
        const coverageData = readFileSync(resolve(this.projectRoot, 'coverage-results.json'), 'utf8')
  ...
  private analyzeFileForTestGap(file: string): TestGap | null {
    try {
      const content = readFileSync(file, 'utf8')
      const fileName = file.replace(this.sourceDir + '/', '')
      
      // Determine file type and priority
      let type: TestGap['type']
      let priority: TestGap['priority']
      let estimatedTests: number
  ...
  private checkTestCompleteness(sourceFile: string, testFile: string): TestGap | null {
    try {
  ...
  private getFileType(file: string): TestGap['type'] {
  ...
  private countReactComponents(content: string): number {
  ...
  private identifyMissingTestTypes(content: string, type: TestGap['type']): string[] {
  ...
  private generateRecommendations(gaps: TestGap[], coverage: CoverageReport | null): string[] {
  ...
  private calculateMetrics(gaps: TestGap[], coverage: CoverageReport | null): {
    totalFiles: number
    testedFiles: number
    missingTests: number
    coverageScore: number
  ...
  generateReport(analysis: any): string {
...
export { TestCoverageAnalyzer, COVERAGE_THRESHOLDS }